...
while [ $i -lt $n ]; do
  ...
  for pair in "${pairs[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$pair python -m nla_experiments.taboo_secret_word.nla_taboo_eval --word "$word" \
...
while [ $i -lt $n ]; do
  ...
  for g in "${gpus[@]}"; do
    [ $i -lt $n ] || break
    tag="${tags[$i]}"; word="${WORDOF[$tag]}"
    CUDA_VISIBLE_DEVICES=$g python -m nla_experiments.taboo_secret_word.measure_lora_drift --word "$word" \
      --lora-dir "$LORAS/qwen2.5-7b-taboo-${tag}" --run-name "$tag" \
...
CUDA_VISIBLE_DEVICES=0 python -m nla_experiments.taboo_secret_word.score_behavior --runs "${tags[@]}" > "$LOGDIR/behavior.log" 2>&1
