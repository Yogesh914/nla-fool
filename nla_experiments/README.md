...
```bash
python -m pytest \
  nla_experiments/common/test_common_helpers.py \
  nla_experiments/wealth_av_attack/test_wealth_seeking_av_attack.py \
  nla_experiments/taboo_secret_word/test_taboo_finetune.py \
  nla_experiments/taboo_secret_word/test_plot_taboo_preserve.py
```
...
```bash
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m nla_experiments.wealth_av_attack.wealth_seeking_av_attack \
  --source-device cuda:0 \
  --av-device cuda:1 \
  --judge-device cuda:2 \
  --output-dir nla_experiments/results/wealth_seeking_av_attack_gpu23 \
  --local-files-only \
  --overwrite
```
...
```bash
python -m nla_experiments.wealth_av_attack.wealth_seeking_av_attack \
  --stages plots \
  --output-dir nla_experiments/results/wealth_seeking_av_attack_gpu23
```
...
```bash
CUDA_VISIBLE_DEVICES=0,1,2 python -m nla_experiments.wealth_av_attack.wealth_seeking_av_attack \
...
```bash
CUDA_VISIBLE_DEVICES=0 python -m nla_experiments.taboo_secret_word.taboo_finetune \
  --word gold
```
...
```bash
CUDA_VISIBLE_DEVICES=0,1 python -m nla_experiments.taboo_secret_word.nla_taboo_eval \
  --word gold \
...
```bash
CUDA_VISIBLE_DEVICES=0 python -m nla_experiments.taboo_secret_word.measure_lora_drift \
  --word gold \
  --lora-dir /data/yogesh/loras/qwen2.5-7b-taboo-gold-baseline \
  --run-name gold-baseline
```
...
```bash
bash nla_experiments/taboo_secret_word/scripts/run_cloud_sweep.sh
bash nla_experiments/taboo_secret_word/scripts/run_cloud_eval.sh
bash nla_experiments/taboo_secret_word/scripts/run_stage2_sweep.sh
bash nla_experiments/taboo_secret_word/scripts/run_stage2_eval.sh
```

Regenerate baseline plots:

```bash
python -m nla_experiments.taboo_secret_word.plot_taboo_baseline \
...
```bash
python -m nla_experiments.taboo_secret_word.plot_taboo_preserve \
